...
bool isIntersecting(Tree T, clustering& cl, dataset& initialDt, dataset& currentDt, bool univ, bool updateCl){
  ...
  for (int t=0; t<T.N; t++){    
	...
	sum_sj_star += T.a[t*initialDt.J+j] != 0.0;
      }

      int nbPoints = 0;
      for (int i=0; i<IL[t].size(); i++){
	nbPoints += cl.clusters[IL[t][i]].nb;
      }
      for (int i=0; i<IR[t].size(); i++){
	nbPoints += cl.clusters[IR[t][i]].nb;
      }
	...
	name_a[j] = "a" + to_string(j);
      ...
      GRBLinExpr sum_sj = 0;
      for (int j=0; j<initialDt.J;j++){ // -sj <= aj <= sj
	...
	for (int i=0; i<cl.clusters[IL[t][g]].nb; i++){
	...
	for (int i=0; i<cl.clusters[IR[t][g]].nb; i++){
	  GRBLinExpr sum_ax_i = 0;
	  for (int j=0; j<initialDt.J; j++){
  ...
  if (updateCl and B){
    ...
    T.predictLeaves(initialDt, leavesOfAllPoints);
    for (int g=0; g<currentDt.I; g++){
      if (isCut[g].size()>0){
	vector<vector<int>> division;
	map<int,int> leafOfDiv;
	for (int i=0; i<cl.clusters[g].nb; i++){
	...
	//cout << "Taille initiale: " << cl.clusters.size() << " + nouvelle div: " << division.size();
	cl.breakCluster(g,division,initialDt);
	//cout << "    taille nouveau clustering: " <<cl.clusters.size()<< endl;
      }
    }
  }
...
solClust decontractingAlgorithm(dataset& dt, clustering& cl, model_type modelt, parameters p, int timeL){
  ...
  while(keepGoing and clust.clusters.size() < dt.I){
    ...
    build_model model = build_model(md,currentDt,modelt,p);
    solution sol = model.solve(md);
