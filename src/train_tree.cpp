...
training_results_cl learningWithClustering(dataset& dt_train, dataset& dt_validation, dataset& dt_test, clustering cl, baseModel bm, bool univ, int DMAX, double time_limit, int Nmin){
  ...
  model_type mt = model_type(bm, univ, false, true, false, true);
  ...
  for (int D=1; D<=DMAX; D++){
    ...
    while (C >= D){
      ...
      solClust solC = iteratingOTP(dt_train, cl, mt, p, time_limit);

      int bestTree = solC.T.size()-1;
      if (solC.pseudoGap > 0){
	for (int t=0; t<solC.T.size()-1; t++){
      ...
      string itOptName = "../TreesAndPartitions/"+dt_train.name+"/itOpt2_"+model_name(bm,univ)+"_"+cl.name+"_part"+to_string(dt_train.partition)+"_D"+to_string(D)+"_C"+to_string(nb_br)+".txt";
      solC.write(itOptName);

      string treeName = "../TreesAndPartitions/"+dt_train.name+"/sol2_"+model_name(bm,univ)+"_"+cl.name+"_part"+to_string(dt_train.partition)+"_D"+to_string(D)+"_C"+to_string(nb_br)+".txt";
      ...
      tr.globalNbOpti += (int)(solC.pseudoGap==0);
