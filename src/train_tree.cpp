...
training_results learning_Bertsimas(dataset& dt_train, dataset& dt_validation, dataset& dt_test, baseModel bm, bool univ, int DMAX, double time_limit, int Nmin, double mu){
  ...
  tr.perc_err[0] = dom_trees.trees[best].Et / dt_test.I;
...
training_results learning(dataset& dt_train, dataset& dt_validation, dataset& dt_test, baseModel bm, bool univ, int DMAX, double time_limit, int Nmin, double mu){
  ...
  for (int D=1; D<=DMAX; D++){
	...
	sol.T.post_processing_a_b(dt_train, bm!=baseModel::F);
      ...
      string postProcessedTreeName = "../TreesAndPartitions/"+dt_train.name+"/sol_"+model_name(bm,univ)+"_part"+to_string(dt_train.partition)+"_D"+to_string(D)+"_C"+to_string(C)+"_PP.txt";
  ...
  tr.perc_err[0] = dom_trees_s_pp.trees[best[0]].Et / dt_test.I;
  ...
  tr.perc_err[1] = dom_trees_s_pp.trees[best[1]].Et / dt_test.I;
  ...
  tr.perc_err[2] = dom_trees_a_pp.trees[best[2]].Et / dt_test.I;
  ...
  tr.perc_err[3] = dom_trees_a_pp.trees[best[3]].Et / dt_test.I;
